...
class training_model:
    ...
    '''
    def __init__(self,X, Y,layers_dimenstions,activation_functions, alpha=0.7 , no_of_iterations=100, print_cost = True, lambd = 0  ):
        '''
    ...
    def update_parameters(self,parameters, grads, learning_rate):
        '''
          "The function which is used to update the weights and biases of the model ."
    ...
    def train(self):
        ...
        '''
        layers_dimensions = [self.input.shape[0], 128, self.Y.shape[0]]
        ...
        for i in range(0,self.no_of_iterations):
            ...
            grads = backward_model.model_backward_general(predictions, self.Y, packet_of_packets, "multiclass",self.regularization_parameter).model_backward()
            # Update parameters.
...
#print("y_label:")
#print(train_Y.shape)
activation_functions = ["NONE","sigmoid","relu"]
parameters = training_model(train_X.T, train_Y,[],activation_functions).train()


#parameters = training_model(X_train, Y_train).train()
temp=evaluation_metrics(temp,train_X.T,parameters).Accuracy(train_X.shape[0])
